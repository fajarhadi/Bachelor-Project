...
<<<<<<< HEAD
	...
	latlng += "/";
  latlng += String(kalAngleX,3);
  latlng += "/";
  latlng += String(kalAngleY,3);
	latlng += "/";
...
<<<<<<< HEAD
  ...
  kalmanX.setAngle(a); // Set starting angle
  kalmanY.setAngle(b);
  kalmanZ.setAngle(c);
  kalmanX.setQbias(45);kalmanY.setQbias(78);kalmanZ.setQbias(23);
...
{  
  //get_GPS();
  static uint32_t period = 0.2 * 60000;
  uint32_t tStart = millis();
  timer2 = micros();
  double dt2 = (double)(micros() - timer2) / 1000000;
/*
  while ((millis() - tStart) < period)
    ...
    float yaw = mpu.getYaw();
		...
		kalAngleX = kalmanX.getAngle(rolls, gyroXrate, dt2);
		kalAngleY = kalmanY.getAngle(pitchs, gyroYrate, dt2);

    double dt = (double)(millis() - timer) / 1000000;
    timer = millis();
    fLat = atof (latitude);
    fLong = atof (logitude);
    fSpeed = atof (speedOTG);
    kXpost = kalmanLong.getAngle(fLong, yaw, dt);
    kYpost = kalmanLat.getAngle(fLat, yaw, dt);
	}
	sendData(dt2);*/
  mpu.update();
  float a = mpu.getAccX();
  ...
  kalAngleX = kalmanX.getAngle(a, a, dt2);
	kalAngleY = kalmanY.getAngle(b, b, dt2);
  kalAngleZ = kalmanZ.getAngle(c, c, dt2);

  float d = sqrt(pow(kalAngleX, 2) + pow(kalAngleY, 2) + pow(kalAngleZ, 2)) - 0.98;
  d *= 9.80655;
